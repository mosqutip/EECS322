  ...
  (let* ([conf-colors (set-union (list->set (hash-map colorings
                                                (λ (key val)
                                                  (when (set-member? (hash-ref color-hash var) key)
                                                    val)))))]
         [poss-col (set-subtract reg-set (hash-ref color-hash var) conf-colors)])
    (if (set-empty? poss-col) 
        (set! colorable #f)
        (hash-set! colorings var (list-ref (set->list poss-col) 0)))))
...
; print our adjacency and color maps
(print-adjacency)
(print-coloring)
